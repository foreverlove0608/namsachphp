...
class action_menu extends library{
    ...
    /*---------- LAY 2 MENU CHO MENU PHAI CUA TRANG NAM SACH ------------*/
    public function getListMenu_right($valueCol_id){
        global $conn_vn;
        
        $stateOrder_menu='ASC';

        $rows = array();
        $sql = "SELECT * FROM $this->nameTable_menu WHERE $this->nameColParent_menu = '".$valueCol_id."' Order By menu.menu_sort_order ASC limit 4,6"; 
        //echo $sql;
    ...
    /*---------- LAY 4 MENU CHO MENU BOT CUA TRANG NAM SACH ------------*/
    public function getListMenu_bot($valueCol_id){
        global $conn_vn;
        
        $stateOrder_menu='ASC';

        $rows = array();
        $sql = "SELECT * FROM $this->nameTable_menu WHERE $this->nameColParent_menu = '".$valueCol_id."' Order By menu.menu_sort_order ASC limit 4"; 
        //echo $sql;
        $result = mysqli_query($conn_vn,$sql);
        while($row = mysqli_fetch_array($result)){
                $rows[] = $row;
